...
async function loadPaymentMethods() {
    ...
    try {
        ...
        if (paymentMethods && paymentMethods.length > 0) {
                ...
                methodElement.innerHTML = `
                    <div class="payment-method-info">
                        ...
                        <div class="payment-method-details">
                            ...
                            <div class="payment-method-number">**** **** **** ${method.lastFour}</div>
                            <div class="payment-method-expiry">Expire: ${method.expiry}</div>
                        </div>
                    </div>
                    <div class="payment-method-actions">
                        ...
                        <button class="payment-action-btn delete" data-id="${method.id}"><i class="fas fa-trash"></i></button>
                    </div>
                `;
            ...
            paymentMethodsList.innerHTML = '<div class="no-data">ไม่พบข้อมูลวิธีการชำระเงิน</div>';
        }
    } catch (error) {
        console.error('เกิดข้อผิดพลาดในการโหลดข้อมูลวิธีการชำระเงิน:', error);
        paymentMethodsList.innerHTML = '<div class="no-data">ไม่สามารถโหลดข้อมูลวิธีการชำระเงินได้</div>';
    }
}
...
function setupMembershipTabs() {
    ...
    const tabContents = document.querySelectorAll('.membership-tabs .tab-content');
...
function setupChangePlanButton() {
    ...
    selectPlanButtons.forEach(button => {
        button.addEventListener('click', async function() {
            ...
            if (confirm(`คุณต้องการเปลี่ยนแผนสมาชิกเป็น ${planId.charAt(0).toUpperCase() + planId.slice(1)} หรือไม่?`)) {
                try {
                    // ในระบบจริงจะเรียกใช้ API
                    ...
                    if (response.success) {
                        ...
                        // อัพเดทข้อมูลในหน้า
                        document.getElementById('membership-type-value').textContent = response.newPlan.type;
                        
                        // อัพเดทสิทธิประโยชน์ตามแผนที่เลือก
                        updateBenefitsByPlan(planId);
                        
                        // เพิ่มประวัติการเปลี่ยนแปลง
                        addMembershipHistoryItem('Plan Changed', `Changed to ${response.newPlan.type} plan`);
                    ...
                    showNotification('ไม่สามารถเปลี่ยนแผนสมาชิกได้ กรุณาลองใหม่อีกครั้ง', 'error');
                }
            }
        });
    });
...
function setupCancelMembershipButton() {
    ...
    cancelButton.addEventListener('click', async function() {
        if (confirm('คุณแน่ใจหรือไม่ว่าต้องการยกเลิกสมาชิก? การกระทำนี้จะเปลี่ยนแผนของคุณเป็น Standard')) {
            try {
                // ในระบบจริงจะเรียกใช้ API
                ...
                if (response.success) {
                    ...
                    // อัพเดทข้อมูลในหน้า
                    document.getElementById('membership-type-value').textContent = 'Standard';
                    
                    // อัพเดทสิทธิประโยชน์
                    updateBenefitsByPlan('standard');
                    
                    // เพิ่มประวัติการเปลี่ยนแปลง
                    addMembershipHistoryItem('Membership Downgraded', 'Changed to Standard plan');
                ...
                showNotification('ไม่สามารถยกเลิกสมาชิกได้ กรุณาลองใหม่อีกครั้ง', 'error');
            }
        }
    });
}
...
function setupAddPaymentButton() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            showNotification('ไม่สามารถเพิ่มวิธีการชำระเงินได้ กรุณาลองใหม่อีกครั้ง', 'error');
        }
    });
}
...
function setupPaymentMethodActions() {
    ...
    const setDefaultButtons = document.querySelectorAll('.payment-action-btn.set-default');
        ...
        button.addEventListener('click', async function() {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    throw new Error(response.message || 'ไม่สามารถตั้งค่าวิธีการชำระเงินเริ่มต้นได้');
                }
            } catch (error) {
                console.error('เกิดข้อผิดพลาดในการตั้งค่าวิธีการชำระเงินเริ่มต้น:', error);
                showNotification('ไม่สามารถตั้งค่าวิธีการชำระเงินเริ่มต้นได้ กรุณาลองใหม่อีกครั้ง', 'error');
            }
        });
    });
            ...
            if (confirm('คุณแน่ใจหรือไม่ว่าต้องการลบวิธีการชำระเงินนี้?')) {
                try {
                    ...
                    if (response.success) {
                        ...
                        throw new Error(response.message || 'ไม่สามารถลบวิธีการชำระเงินได้');
                    }
                } catch (error) {
                    console.error('เกิดข้อผิดพลาดในการลบวิธีการชำระเงิน:', error);
                    showNotification('ไม่สามารถลบวิธีการชำระเงินได้ กรุณาลองใหม่อีกครั้ง', 'error');
                }
            }
        });
    });
